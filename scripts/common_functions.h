...
TLine* draw_line(double x1, double x2, double y1, double y2, int width=2, int style=1, int color=1) {
...
TLatex* prelim_lat_(double xmin, double xmax, double ymin, double ymax, bool in) {
  TLatex* cms_lat = 0;
  std::string prelim = "CMS #scale[0.8]{#font[52]{Preliminary}}";// #font[22]{Times bold} and #font[12]{Times Italic}
  if (in) { cms_lat = new TLatex(xmin+(xmax-xmin)/20.0, ymax-(ymax-ymin)/10.0, prelim.c_str()); cms_lat->SetLineWidth(2); cms_lat->Draw(); }
  else { cms_lat = new TLatex(xmin, ymax+(ymax-ymin)/40.0, prelim.c_str()); cms_lat->SetLineWidth(2); cms_lat->Draw(); }
  return cms_lat;
}

TLatex* cms_lat_(double xmin, double xmax, double ymin, double ymax, bool in) {
  TLatex* cms_lat = 0;
  std::string prelim = "CMS";// #font[22]{Times bold} and #font[12]{Times Italic}
  if (in) { cms_lat = new TLatex(xmin+(xmax-xmin)/20.0, ymax-(ymax-ymin)/10.0, prelim.c_str()); cms_lat->SetLineWidth(2); cms_lat->Draw(); }
  else { cms_lat = new TLatex(xmin, ymax+(ymax-ymin)/40.0, prelim.c_str()); cms_lat->SetLineWidth(2); cms_lat->Draw(); }
  return cms_lat;
}

TLatex* era_lat_(double xmin, double xmax, double ymin, double ymax) {
  std::string prelim = "119.5 fb^{-1} (2017+2018, 13 TeV)";// #font[22]{Times bold} and #font[12]{Times Italic}
...
TLatex* era2_lat_(double xmin, double xmax, double ymin, double ymax) {
...
void remove_points(TGraphAsymmErrors* g, int first, int last) {
  for (int i=last; i>=first; --i) g->RemovePoint(i);
}
...
void remove_plot(TCanvas* c, int iplot) {
  ...
  bool tframe = TString(c->GetListOfPrimitives()->At(0)->GetName()).Contains("TFrame");
  if (leg) leg->GetListOfPrimitives()->RemoveAt(iplot+1);
  c->GetListOfPrimitives()->RemoveAt(iplot+tframe);
}
...
TLegend* modify_legend(TCanvas* c, std::string title, double x1, double x2, double y1, double y2) {
  TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    //std::cout<<i<<" "<<c->GetListOfPrimitives()->At(i)->GetName()<<std::endl;
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
  }
...
TLegend* rename_legend(TCanvas* c, std::string title) {
  TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
...
TLegend* move_legend(TCanvas* c, double x1, double y2) {
TLegend *leg = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) {
    if (std::string(c->GetListOfPrimitives()->At(i)->GetName())=="TPave") 
      leg = (TLegend*)c->GetListOfPrimitives()->At(i);
...
void add_labels_Phase1(TCanvas* c, double slope = 0) {
  TH1D *h = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) if (h==0) {
    if (TString(c->GetListOfPrimitives()->At(i)->GetName()).Contains("_vs_IntLumi2017")) h = (TH1D*)c->GetListOfPrimitives()->At(i);
  }
  double xmin = h->GetXaxis()->GetBinLowEdge(h->GetXaxis()->GetFirst());
  double xmax = h->GetXaxis()->GetBinUpEdge(h->GetXaxis()->GetLast());
  double ymin = h->GetMinimum();
  double ymax = h->GetMaximum();
  const double yearLimitX =  52;
  TLine *line;
  ...
  //line->Draw();
  TText  *t[2];
  t[0] = new TText( 2,ymax-0.04*(ymax-ymin),"2017");
  t[1] = new TText(54,ymax-0.04*(ymax-ymin),"2018");
  for (int i=0;i<2;++i){
    t[i]->SetTextAlign(12);
    t[i]->SetTextColor(kGray+4);
    t[i]->SetTextFont(62);
  ...
  //TText *calib[6];
  //double ytext = ymin + (ymax-ymin)*0.05;
  //calib[0] = new TLatex( 11.8, ytext, "HV #color[618]{L2(250V)}");
  //calib[1] = new TLatex( 20.3, ytext, "HV #color[633]{L1(350V)} #color[601]{L3}/#color[434]{4}#color[601]{(20}#color[434]{0V)}");
  //calib[2] = new TLatex( 51.8, ytext, "HV #color[633]{L1}-400V");
  //calib[3] = new TLatex( 75.2, ytext, "HV #color[618]{L2}-300V");
  //calib[4] = new TLatex(106.8, ytext, "HV #color[633]{L1}-450V");
  //for (int i=0;i<5;++i){
  //  calib[i]->SetTextAlign(12);
  //  calib[i]->SetTextFont(62);
  //  calib[i]->SetTextSize(0.03);
  //  calib[i]->SetTextAngle(90);
  //  calib[i]->Draw();
  //}
  std::vector<double> hv  = {11.8, 19.9, 20.3, 20.7, 51.8, 74.8, 75.2, 75.6, 106.8};
  std::vector<int>    col = {618,  633,  601,  434,  633,  618,  601,  434,  633};
  for (size_t i=0;i<hv.size();++i){
    line = new TLine (hv[i],ymin, hv[i], ymax);
    line->SetLineStyle(2);
    line->SetLineColor(col[i]);
    line->SetLineWidth(2);
    line->Draw();
  }
  std::vector<double> gains = {6.4, 24.8, 52.1, 75.5, 80.9, 83.1, 107.0, 119.5};
  for (size_t i=0;i<gains.size();++i){
    line = new TLine (gains[i],ymin, gains[i], ymax);
    line->SetLineStyle(9);
    line->SetLineColor(1);
    line->SetLineWidth(1);
    line->Draw();
  }
  TLine* hvline = new TLine(0,1,0,1);
  hvline->SetLineStyle(2);
  hvline->SetLineWidth(2);
  TLegend *leg = new TLegend(0.71,0.15,0.91,0.25,"");
  leg->AddEntry(line,   "Gain change", "L");
  leg->AddEntry(hvline, "HV change",   "L");
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
...
void add_labels_Phase1_fpix(TCanvas* c, double slope = 0) {
  TH1D *h = 0;
  for (int i=0, n=c->GetListOfPrimitives()->GetEntries(); i<n; ++i) if (h==0) {
    if (TString(c->GetListOfPrimitives()->At(i)->GetName()).Contains("_vs_IntLumi2017")) h = (TH1D*)c->GetListOfPrimitives()->At(i);
  }
  ...
  std::vector<double> hv  = {20.3, 106.8};
...
TF1* fit_turnon_improved_(TH1 *h, TH1* dist, bool draw=0) {
