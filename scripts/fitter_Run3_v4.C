...
void fitter_Run3_v4() {
  ...
  //  g4->RemovePoint(i);
  //}
  // Add an outlier point so be able to use larger ranges
  int n= g1->GetN();
  g1->Expand(n+1);
  g1->SetPoint(n,100,-1);
  g1->GetXaxis()->SetTitle("Instantaneous Luminosity (#scale[0.8]{#times10^{33}cm^{-2}s^{-1}})");
  ...
  g1_out->Draw("SAMEP");
  g1->GetXaxis()->SetRangeUser(0,25);
  g1->GetYaxis()->SetRangeUser(0.7,1);
  g1->GetXaxis()->SetLabelSize(0.035);
  g1->GetYaxis()->SetLabelSize(0.035);
  float maxfit = 14;
  TF1 *fit_l1_in  = new TF1("fit_l1_in", "pol3",0,maxfit);
  TF1 *fit_l1_out = new TF1("fit_l1_out","pol3",0,maxfit);
  TF1 *fit_l2 = new TF1("fit_l2","pol1",0,25);
  TF1 *fit_l3 = new TF1("fit_l3","pol1",0,25);
  TF1 *fit_l4 = new TF1("fit_l4","pol1",0,25);
  fit_l1_in ->FixParameter(0,1);
  fit_l1_out->FixParameter(0,1);
  fit_l1_in ->FixParameter(3,-1.4e-5);
  fit_l1_out->FixParameter(3,-1.1e-5);
  ...
  g3->Fit("fit_l3","RM");
  g4->Fit("fit_l4","RM");
  fit_l1_in->SetRange(0,25);
  fit_l1_out->SetRange(0,25);
  fit_l1_in->Draw("SAME");
  fit_l1_out->Draw("SAME");
  // Get dcol efficiency
  TF1 *dcol_l1_in  = new TF1("dcol_l1_in", "pol3",0,25);
  TF1 *dcol_l1_out = new TF1("dcol_l1_out","pol3",0,25);
  TF1 *dcol_l2 = new TF1("dcol_l2","pol1",0,25);
  TF1 *dcol_l3 = new TF1("dcol_l3","pol1",0,25);
  TF1 *dcol_l4 = new TF1("dcol_l4","pol1",0,25);
  ...
  leg->Clear();
  leg->SetNColumns(2);
  leg->SetHeader("v6, L1 linear extrapolation [0,14]");
